...
"""
This version has the better softmax correlation module with double softmax
"""
...
class RegTR(GenericRegModel):
    ...
    def forward(self, batch):
        ...
        if self.verbose:
            print("\n")
            print(f"type(batch['src_xyz']): {type(batch['src_xyz'])}")
            print(f"batch['src_xyz'].shape: {batch['src_xyz'].shape}")

            print(f"type(batch['tgt_xyz']): {type(batch['tgt_xyz'])}")
            print(f"batch['tgt_xyz'].shape: {batch['tgt_xyz'].shape}")
