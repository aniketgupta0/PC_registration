...
from utils.se3_torch import compute_rigid_transform, se3_transform_list, se3_transform, se3_inv, compute_rigid_transform_with_sinkhorn, compute_rigid_transform_with_sinkhorn_2
...
class RegTR(GenericRegModel):
    ...
    def forward(self, batch):
        ...
        if self.verbose:
            ...
            print("src_overlap_list dimensions are", src_overlap_list[1].shape)
    ...
    def compute_loss(self, pred, batch):
        ...
        # conf_matrix = confusion_matrix(all_overlap_gt.cpu().numpy()>0.5, all_overlap_pred[0, :, 0].cpu().numpy()>0.5)
        ...
        # self.pos_class.append((all_overlap_gt.cpu().numpy()>0.5).sum())
        # self.neg_class.append((all_overlap_gt.cpu().numpy()<0.5).sum())
        # print(conf_matrix)
        ...
        if self.verbose:
            ...
            print(f"Inlier loss: {inlier_loss}")
        ...
        for i in range(B):
            ...
            if N>M:
                ...
                if self.cfg.remove_points_from_val:
                    val, ind = torch.topk(val, int(0.15*M), dim=1)
                ...
                # print(val.shape)
                if self.cfg.remove_points_from_val:
                    val, ind = torch.topk(val, int(0.15*N), dim=1)
                    src_pts = torch.gather(src_pts, 0, ind.permute(1,0).expand(-1,3))
                    tgt_pts = torch.gather(tgt_pts, 0, ind.permute(1,0).expand(-1,3))
