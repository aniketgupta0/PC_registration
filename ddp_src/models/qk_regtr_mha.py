...
class RegTR(GenericRegModel):
    def __init__(self, cfg, *args, **kwargs):
        ...
        if cfg.get('direct_regress_coor', False):
            ...
            self.correspondence_decoder = CorrespondenceDecoder(cfg.d_embed,
                                                                ...
                                                                self.pos_embed)
        self.multihead_attn = MultiheadAttention(cfg.d_embed, cfg.nhead, dropout=cfg.dropout)
    ...
    def compute_loss(self, pred, batch):
        ...
        for i in range(len(pred['overlap_prob_list'])):
            ...
            if src_overlap_p[i].shape[0]>tgt_overlap_p[i].shape[0]:
                overlap_gt = tgt_overlap_p[i]
            else:
                overlap_gt = src_overlap_p[i]

            overlap_loss += self.overlap_criterion(overlap_pred[:, 0], overlap_gt)
        ...
        losses['total'] = 3 * T_loss + 0.1*feature_loss + 0.2*overlap_loss
        # losses['total'] = T_loss
        return losses
    ...
    def softmax_correlation(self, src_feats, tgt_feats, src_xyz, tgt_xyz):
        ...
        for i in range(B):
            ...
            if N>M:
                # predict target points's correspondence
                src_xyz_p = self.pos_embed(src_xyz[i])
                src_feat, attn_N = self.multihead_attn(tgt_feats[i][0].unsqueeze(1), src_feats[i][0].unsqueeze(1), src_xyz_p.unsqueeze(1))
                src_pts_list, src_overlap_list = self.correspondence_decoder(src_feat, [src_xyz[i]])
                src_pts = src_pts_list[0]
                src_overlap = src_overlap_list[0]
                ...
                # attention = torch.gather(attn_N, 2, ind.unsqueeze(-1).expand(-1, M, -1))
                ...
                overlap_prob_list.append(src_overlap)
                # ind_list.append(ind.squeeze())

            else:
                tgt_xyz_p = self.pos_embed(tgt_xyz[i])
                tgt_feat, attn_M = self.multihead_attn(src_feats[i][0].unsqueeze(1), tgt_feats[i][0].unsqueeze(1), tgt_xyz_p.unsqueeze(1))
                tgt_pts_list, tgt_overlap_list = self.correspondence_decoder(tgt_feat, [tgt_xyz[i]])
                tgt_pts = tgt_pts_list[0]
                tgt_overlap = tgt_overlap_list[0]
                # attn_M = torch.nn.functional.softmax(correlation, dim=-1)
                attn_list.append(attn_M)

                val, ind = torch.max(attn_M, dim=-1)
                ...
                overlap_prob_list.append(tgt_overlap)
                # ind_list.append(ind.squeeze())
...
class CorrespondenceRegressor(nn.Module):
    ...
    def forward(self, src_feats_padded, src_xyz):
        """
        ...
        return src_corr_list, src_overlap_list
