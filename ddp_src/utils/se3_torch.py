...
def compute_rigid_transform_with_sinkhorn_2(xyz_s, xyz_t, affinity, slack, n_iters, mask=None):
    # affinity = -(score_matrix - self.softplus(self.alpha))/(torch.exp(self.beta) + 0.02)
    # affinity = attention_mat
    ...
    # print(f"xyz_s: {xyz_t.shape} ")
    # print(f"weighted_t shape: {weighted_t.shape}")
    transform = compute_rigid_transform_2(xyz_s, xyz_t, weights=torch.sum(perm_matrix, dim=2))
