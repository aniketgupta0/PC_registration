...
from models.transformer.transformer_only_ce import TransformerCrossEncoderLayer, TransformerCrossEncoder
...
"""
This version is where we use positional embeddings on all revvit layers 
also but after the first layer.
So the archtiecture will be as follows:
Revvit_1: no pos_embed
Revvit_2-Revvit_n: yes pos_embed
ce_layers1-ce_layers_n: yes pos_embed
...
class RegTR(GenericRegModel):
    def __init__(self, cfg, *args, **kwargs):
        ...
        ################################
        self.rev_T_input_layer = nn.ModuleList([ReversibleBlock(dim=cfg.rev_dim, num_heads=cfg.rev_head)])
    ...
    def forward(self, batch):
        ...
        #####################################
        # Reversible Transformer First layer
        #####################################
        tic = time.time()
        src_features = RevBackProp.apply(src_features, self.rev_T_input_layer)
        tgt_features = RevBackProp.apply(tgt_features, self.rev_T_input_layer)

        if self.time_verbose:
            print(f"Time for first Reversible transformer layer: {time.time()-tic}")
        if self.verbose:
            print("After the first reversible transformer layer")
            print(f"src_features shape is: {src_features.shape}")
            print(f"tgt_features shape is: {tgt_features.shape}")
        ...
        #################################
        tic = time.time()
        src_feats_un = RevBackProp.apply(src_features + src_pe, self.rev_T_layers)
        tgt_feats_un = RevBackProp.apply(tgt_features + tgt_pe, self.rev_T_layers)
